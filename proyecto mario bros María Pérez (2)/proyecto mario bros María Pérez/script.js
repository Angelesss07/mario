...
let marioAltura = mario.style.top  ;
let moneda = document.querySelector(".moneda");  // Elemento HTML de una moneda
let hongo = document.querySelector(".hongo")
...
const gameOver = new Audio("./sonidos/Die.wav"); // Música de fondo
...
function derecha() {
  if (derechaPresionado) {
    marioDirección = 5;
    let posicionMario = parseInt(mario.style.left) || 75 ;
    let nuevaPosicionMario = posicionMario + marioVelocidad * marioDirección;
...
function agarrarMoneda() {
  ...
  if (
    ...
    sonidoMoneda.play(); // Reproduce sonido de muerte
...
function agarrarHongo() {
  ...
  let hongoPos = hongo.getBoundingClientRect();
    if (
      ...
      sonidoHongo.play(); // Reproduce sonido de muerte
...
let boton = document.querySelector(".cambiar-color")
...
botonFinalizarJuego.onclick = function () {
  setTimeout(() => {
   nivel.innerHTML = "Game Over María";
   gameOver.play();
   TemaFondo.pause();
   juegoActivo = false;
...
function detectarColisionGoomba() {
  ...
  if (
    ...
    gameOver.play(); // Reproduce sonido de muerte
